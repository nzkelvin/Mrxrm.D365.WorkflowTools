...
namespace Mrxrm.D365.WorkflowTools.Mrxrm.D365.WorkflowTools
{
    ...
    public class DistributeWorkflowActivityOneToMany: WorkFlowActivityBase, IDistributeWorkflowActivity
    {
        ...
        public IEnumerable<Guid> GetKeys(EntityReference parentRef, string relationshipName, IOrganizationService orgService)
        {
            var relationshipInfo = GetRelationshipInfo(relationshipName, orgService);

            if (!String.Equals(parentRef.LogicalName, relationshipInfo.ReferencedEntity,
                StringComparison.InvariantCultureIgnoreCase))
            {
                throw new InvalidWorkflowException($"The parent entity type ({parentRef.LogicalName}) is different from the relationship referenced entity type {relationshipInfo.ReferencedEntity}.");
            }
            ...
            using (var ctx = new OrganizationServiceContext(orgService))
            {
                var childrenRecords = from r in ctx.CreateQuery(relationshipInfo.ReferencingEntity)
                    where ((EntityReference)r[relationshipInfo.ReferencingAttribute]).Id == parentRef.Id
                    select r.Id;
